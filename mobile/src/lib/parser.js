// ─────────────────────────────────────────────────────────────────────────────
// Blood report parsing engine — ported from the web app's app.js.
// Pure functions only: no DOM, no framework. Same behavior, ESM-bundled PDF.js
// worker instead of a CDN (the app must work fully offline).
// ─────────────────────────────────────────────────────────────────────────────
...
export function extractValueAndRef(lineItems, alias, colMap) {
  let markerEndX = 0;
  if (alias) {
    let accumulated = '';
...
// ─────────────────────────────────────────────────────────────────────────────
export function peekNextValue(allLines, i, colMap) {
  for (let j = i + 1; j <= Math.min(i + 2, allLines.length - 1); j++) {
    const next = allLines[j];
    if (next.pageBreak) break;
...
// ─────────────────────────────────────────────────────────────────────────────
// Parse a single PDF.
// Unlike the web app, this never blocks on window.prompt — if no date can be
// detected, `date` comes back null and the caller (UI layer) asks the user.
// ─────────────────────────────────────────────────────────────────────────────
